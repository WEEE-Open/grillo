...
export default {
	...
	computed: {
		...
		isEventFormValid() {
			return (
				this.inputRules.length === 0 && this.startTimeRule.length == 0 && this.endTimeRules == 0
			);
		},
	},
	methods: {
	...
	<v-main class="position-relative">
			...
			<v-list v-if="events && events.length > 0">
				...
				>
					...
					<v-list-item-subtitle>
						...
						<br />
						<small
							>{{ formatDateTime(event.startTime) }} - {{ formatDateTime(event.endTime) }}</small
						>
					</v-list-item-subtitle>
		...
		<v-dialog v-model="detailDialog" max-width="600">
			<v-card v-if="selectedEvent">
				...
				<v-card-actions>
					<v-btn variant="text" @click="closeEventDetails"> Close </v-btn>
					<v-spacer></v-spacer>
					<v-btn
						color="primary"
						variant="elevated"
					...
					<v-btn variant="text" icon="mdi-delete" @click="confirmDelete(selectedEvent)" />
				</v-card-actions>
			</v-card>
		</v-dialog>
		...
		<v-dialog v-model="dialog" max-width="600" persistent>
			<v-card>
				...
				<v-card-subtitle v-if="!isEditing"> Create a new event </v-card-subtitle>
				...
				<v-card-text>
					<v-container>
						<v-row>
							...
							<v-col cols="12">
								<v-textarea v-model="record.description" label="Description" variant="outlined" />
							</v-col>
		...
		<!-- Add Button -->
		<v-btn color="primary" size="large" icon elevation="6" class="floating-add-btn" @click="add">
